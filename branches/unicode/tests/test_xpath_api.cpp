...
TEST_XML(xpath_api_select_nodes, "<node><head/><foo/><foo/><tail/></node>")
{
	...
	xpath_node_set ns1 = doc.select_nodes(T("node/foo"));

	xpath_query q(T("node/foo"));
...
TEST_XML(xpath_api_select_single_node, "<node><head/><foo/><foo/><tail/></node>")
{
	...
	xpath_node n1 = doc.select_single_node(T("node/foo"));
	...
	xpath_node n3 = doc.select_single_node(T("node/bar"));
	
	CHECK(!n3);

	xpath_node n4 = doc.select_single_node(T("node/head/following-sibling::foo"));
	xpath_node n5 = doc.select_single_node(T("node/tail/preceding-sibling::foo"));
...
{
	generic_bool_ops_test(doc.select_single_node(T("node")));
	generic_bool_ops_test(doc.select_single_node(T("node/@attr")));
}

TEST_XML(xpath_api_node_eq_ops, "<node attr='value'/>")
{
	generic_eq_ops_test(doc.select_single_node(T("node")), doc.select_single_node(T("node/@attr")));
}

TEST_XML(xpath_api_node_accessors, "<node attr='value'/>")
{
	xpath_node null;
	xpath_node node = doc.select_single_node(T("node"));
	xpath_node attr = doc.select_single_node(T("node/@attr"));

	CHECK(!null.node());
	CHECK(!null.attribute());
	CHECK(!null.parent());

	CHECK(node.node() == doc.child(T("node")));
	CHECK(!node.attribute());
	CHECK(node.parent() == doc);

	CHECK(!attr.node());
	CHECK(attr.attribute() == doc.child(T("node")).attribute(T("attr")));
	CHECK(attr.parent() == doc.child(T("node")));
}
...
inline void xpath_api_node_accessors_helper(const xpath_node_set& set)
{
	...
	CHECK(!set.empty());
	CHECK_STRING(set[0].node().name(), T("foo"));
	CHECK_STRING(set[1].node().name(), T("foo"));
	CHECK(!set[2]);
...
TEST_XML(xpath_api_nodeset_accessors, "<node><foo/><foo/></node>")
{
	...
	xpath_node_set set = doc.select_nodes(T("node/foo"));
...
{
	xpath_query q(T("node/@attr"));

	CHECK(q.evaluate_boolean(doc));
	CHECK(q.evaluate_number(doc) == 3);
	CHECK(q.evaluate_string(doc) == T("3"));

	xpath_node_set ns = q.evaluate_node_set(doc);
	CHECK(ns.size() == 1 && ns[0].attribute() == doc.child(T("node")).attribute(T("attr")));
}
...
{
	CHECK(xpath_query(T("node")).return_type() == xpath_type_node_set);
	CHECK(xpath_query(T("1")).return_type() == xpath_type_number);
	CHECK(xpath_query(T("'s'")).return_type() == xpath_type_string);
	CHECK(xpath_query(T("true()")).return_type() == xpath_type_boolean);
}
